namespace Controls
{
	...
	vec4 DrawTag(const vec4 &in rect, const string &in text, const vec4 &in color = TAG_COLOR, const vec2 &in textOffset = vec2())
	{
		auto dl = UI::GetWindowDrawList();
		dl.AddRectFilled(rect, color, TAG_ROUNDING);
		dl.AddText(vec2(rect.x, rect.y) + TAG_PADDING + textOffset, vec4(1, 1, 1, 1), text);
		return rect;
	}
	...
	void Tag(const vec2 &in size, const string &in text, const vec4 &in color = TAG_COLOR)
	{
		...
		UI::Dummy(textSize + TAG_PADDING * 2);
		DrawTag(UI::GetItemRect(), text, color, textOffset);
	}
